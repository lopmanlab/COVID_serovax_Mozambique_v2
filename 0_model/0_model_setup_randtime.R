...
library(deSolve)
library(ggplot2)
sweep<- readRDS("4_randtime/0_sweep_randtime_serovax.RDS")

source("0_model/0_model_code_sero.R")
spec_humid <- read.csv("0_model/9_spec_humid.csv")[,c("day","avg_sm2")]
start <- readRDS("0_model/9_last_Rrand.RDS")
trigger<-F
sero_e1 <-rep(0,5)
yroot1 <- rep(0,5)
yroot2 <- rep(0,5)
...
model_sims <- function(i){
  ...
  #####################################
  ##Redoing protection post-infection##
  #####################################
 #                   rel_0v1 <- 0.773; rel_0v2 <- 0.62;  rel_0v3 <- 0.464
#  rel_1v0 <- 0.541; rel_1v1 <- 0.418; rel_1v2 <- 0.335; rel_1v3 <- 0.251
#  rel_1v0c<- 0.335
#  rel_2v0 <- 0.233; rel_2v1 <- 0.180; rel_2v2 <- 0.144; rel_2v3 <- 0.108
#  rel_2v0c<- 0.144
  
  
  

#                    rel_0v1 <- 0.6 ; rel_0v2 <- 0.42;  rel_0v3 <- 0.3
#  rel_1v0 <- 0.541; rel_1v1 <- 0.418 ; rel_1v2 <- 0.335; rel_1v3 <- 0.25
#  rel_1v0c<- 0.335
#  rel_2v0 <- 0.4 ; rel_2v1 <- 0.35 ; rel_2v2 <- 0.25; rel_2v3 <- 0.108
#  rel_2v0c<- 0.144
  
                             rel_0v1<- sweep$rel_0v1[i]; rel_0v2<- sweep$rel_0v2[i]; rel_0v3<- sweep$rel_0v3[i]
  rel_1v0<- sweep$rel_1v0[i];rel_1v1<- sweep$rel_1v1[i]; rel_1v2<- sweep$rel_1v2[i]; rel_1v3<- sweep$rel_1v3[i]
  rel_1v0c<-sweep$rel_1v0c[i];
  rel_2v0<- sweep$rel_2v0[i];rel_2v1<- sweep$rel_2v1[i]; rel_2v2<- sweep$rel_2v2[i]; rel_2v3<- sweep$rel_2v3[i]
  rel_2v0c<- sweep$rel_2v0c[i]
  
  ##Heterogeneity in FOI between seropos and neg
  
  
  foi_sp <- sweep$foi_sp[i]
  foi_sn <- sweep$foi_sn[i]
  
    
  #                  rel_0v1 <- 0.773 ; rel_0v2 <- 0.45;  rel_0v3 <- 0.4
  #rel_1v0 <- 0.541; rel_1v1 <- 0.418 ; rel_1v2 <- 0.335; rel_1v3 <- 0.25
  #rel_1v0c<- 0.335
  #rel_2v0 <- 0.45 ; rel_2v1 <- 0.45 ; rel_2v2 <- 0.25; rel_2v3 <- 0.108
  #rel_2v0c<- 0.144
  ...
  params<-c('beta_c'= beta_c, 'beta_a' = beta_a, 'beta_e' = beta_e,
            ...
            'mu_c'=mu_c, 'mu_a'=mu_a, 'mu_e'=mu_e,
            
            'red_inf_1' = red_inf_1, 'red_inf_2'=red_inf_2,
            
            'rel_0v1' = rel_0v1, 'rel_0v2'= rel_0v2, 'rel_0v3'=rel_0v3,
            'rel_1v0' = rel_1v0, 'rel_1v0c'=rel_1v0c,'rel_1v1'= rel_1v1, 'rel_1v2'=rel_1v2, 'rel_1v3'= rel_1v3,
            'rel_2v0' = rel_2v0, 'rel_2v0c'=rel_2v0c,'rel_2v1'= rel_2v1, 'rel_2v2'=rel_2v2, 'rel_2v3'= rel_2v3,  
            
            'foi_sn'= foi_sn, 'foi_sp'=foi_sp,
            ...
            'st0'=st0, 'st1'=st1, 'st2'=st2, 'st3'=st3, 'st4'=st4, 'st5'=st5, 'st6'=st6, 'st7'=st7,'st8'=st8, 'st9'=st9,
            
             'beta1'=beta1, 'w'=w,
            'sero_thresh' =sero_thresh,
            'sweep_unique'=sweep_unique
