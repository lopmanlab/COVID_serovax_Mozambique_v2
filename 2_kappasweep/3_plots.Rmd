...
inc3 <- readRDS("../1_main0.7/0_res/vaxsero_inc.RDS")%>%filter(sweep_unique<=32)
inc4 <- readRDS("../1_main0.7/0_res/vaxint_inc.RDS")%>%filter(sweep_unique<=32)
inc<- bind_rows(inc1, inc2,inc3,inc4)

inc<-inc%>%filter(sero_thresh %in% c("0","0.5","0.6","0.7","0.8","ann_late", "bi_late"))%>%
      mutate(inc_all = (inc_all/tot_pop)*100)

med<- inc%>%group_by(date,sero_thresh,foi_sp,kappa)%>%
...
p1 <- med %>% 
  ...
  facet_grid(vars(lab1),vars(kappa))+
  plot_theme
...
sero3 <- readRDS("../1_main0.7/0_res/vaxsero_sero.RDS")%>%filter(sweep_unique<=32)
sero4 <- readRDS("../1_main0.7/0_res/vaxint_sero.RDS")%>%filter(sweep_unique<=32)
sero<- bind_rows(sero1, sero2,sero3,sero4)


thresh_line <- data.frame(scen=c("No vax","50% thresh", "60% thresh","70% thresh","80% thresh","Annual","Biennial"),
                          sero_thresh = c("0","0.5","0.6","0.7","0.8","ann_late","bi_late"),
                          thresh_num = c(NA,51,62,71,81,NA,NA))%>%
              mutate(scen = factor(scen, levels = c("No vax", "50% thresh", "60% thresh","70% thresh","80% thresh", "Annual","Biennial")))

sero<-sero%>%filter( sero_thresh %in% c("0","0.5","0.6","0.7","0.8","ann_late", "bi_late"))
sero<-sero%>%select(-sero_all)%>%pivot_longer(cols = sero_c:sero_e, names_to = "sero",values_to="val")

sero_med<- sero%>%
      group_by(date,sero_thresh,sero,foi_sp,kappa)%>%
...
p2 <-  ggplot()+
      ...
      facet_grid(vars(scen),vars(kappa))+
      ylim(0,100) +theme_bw()+
...
imm3 <- readRDS("../1_main0.7/0_res/vaxsero_imm_med.RDS")
imm4 <- readRDS("../1_main0.7/0_res/vaxint_imm_med.RDS")

imm_med<- bind_rows(imm1,imm2,imm3,imm4)%>%
            filter(sero_thresh%in%c("0","0.5","0.60","0.7","0.8","ann_late", "bi_late"))
...
p4 <- imm_med%>%
          ...
          scale_fill_manual(values = c("#FFFFCC",
                                       "#35978F","#80CDC1","#C7EAE5",
                                       brewer.pal(9, "PuBu")[c(7,6,5,4,3,2)]))+
          #facet_wrap(~sero_lab, nrow=1)+
          facet_grid(vars(sero_lab),vars(kappa))+
          theme_classic()+
          xlab("Year") + ylab("Population proportion")+
...
death3 <- readRDS("../1_main0.7/0_res/vaxsero_death.RDS")%>%filter(sweep_unique<=32)
death4 <- readRDS("../1_main0.7/0_res/vaxint_death.RDS")%>%filter(sweep_unique<=32)
death<- rbind(death1, death2,death3,death4)%>%filter(sero_thresh %in% c("0","0.5","0.65","0.8","ann_late", "bi_late"))
...
p9 <- med %>% ggplot()+
  ...
  #ggtitle("10-year epidemic projection")+
  xlab("") + ylab("Cases per 100")+
  facet_grid(vars(lab1),vars(kappa))+plot_theme
