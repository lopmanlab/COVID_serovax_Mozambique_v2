...
nnt_main%>%
  mutate(dt_c_per100000=(deaths_averted_e/old_pop)*100000,
         num_deaths_per100000=(num_deaths/tot_pop)*100000)%>%
  group_by(sero_thresh, foi_sp)%>%
  dplyr::summarise(
...
##Deaths and deaths averted per cap
nnt_main%>%
  mutate(dt_avert_e_per100000=(deaths_averted_e/old_pop)*100000,
         num_deaths_per100000=(num_deaths/tot_pop)*100000)%>%
  #filter(nnt>0)%>%
  #filter(!sweep_unique %in% nnt_neg_rem)%>%
  group_by(sero_thresh,foi_sp)%>%
  dplyr::summarise(
            dt_med = median(num_deaths_per100000, na.rm=T),
            dt_25 = quantile(num_deaths_per100000, probs=0.025, na.rm=T), 
            dt_75 = quantile(num_deaths_per100000, probs=0.975, na.rm=T),
            dt_e_percap_med = median(dt_e_per100000,na.rm=T),
            dt_e_percap_25 = quantile(dt_e_per100000, probs=0.025, na.rm=T),            
            dt_e_percap_75 = quantile(dt_e_per100000, probs=0.975,na.rm=T),
            dt_avert_e_per100000_med=median(dt_avert_e_per100000, na.rm=T),
            dt_avert_e_per100000_25=quantile(dt_avert_e_per100000, probs=0.025,na.rm=T),
            dt_avert_e_per100000_75=quantile(dt_avert_e_per100000, probs=0.975,na.rm=T))
```
